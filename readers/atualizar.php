...
foreach ($campos as $campo => $label) {
    if (!isset($_POST[$campo]) || trim($_POST[$campo]) === '') {
        echo "<h2>Erro</h2><p>O campo {$label} é obrigatório.</p><a href='listar.php'><button>Voltar</button></a>";
        $conn->close();
        exit;
    }
}
...
echo "<h2>Erro ao atualizar cadastro.</h2><p>{$stmt->error}</p><a href='listar.php'><button>Voltar</button></a>";
$stmt->close();
$conn->close();
